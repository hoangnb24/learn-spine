...
for d in ([root/sys.argv[1]] if len(sys.argv)>1 else sorted(root.iterdir())):
 ...
 summary={'productInvocations':sum(r['kind']=='tool-start' for r in rows),'unfinishedInvocations':[r['invocationId'] for r in rows if r['kind']=='tool-start' and not any(e.get('invocationId')==r['invocationId'] and e['kind'] in ['tool','tool-thrown'] for e in rows)],'thrownInvocations':[r for r in rows if r['kind']=='tool-thrown'],'afterStopStarts':sum(r['kind']=='tool-start' and r['afterStop'] for r in rows),'discoveryCalls':'reconcile subject transcript','elapsedMs':stop[-1]['elapsedMs'] if stop else None,'startCount':len(start),'stopCount':len(stop),'setupCount':sum(r['kind']=='setup' for r in rows),'prestartInvocations':sum(r['kind']=='tool-start' and r['elapsedMs'] is None for r in rows),'errors':[{'name':r['name'],'result':r['result']} for r in tools if not r['result']['ok']],'media':media,'visualRubric':'pending independent review','reopen':'pending','manualRescue':'reconcile transcript','tokens':None,'cost':None}
