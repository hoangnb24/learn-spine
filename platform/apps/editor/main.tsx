...
function App() {
  useSyncExternalStore(
    editorRuntime.subscribe,
    editorRuntime.getVersion,
    editorRuntime.getVersion,
  );
  ...
  useEffect(() => {
    ...
    void editorRegistration.then((result) => {
      if (active)
        setStatus(
          result.ok ? "Tools sẵn sàng qua WebMCP" : "WebMCP chưa khả dụng",
        );
    });
    return () => {
      active = false;
  ...
  return (
    <>
      <Editor agentStatus={status} />
      {download && (
        <aside
          aria-label="Kết quả từ tools"
          style={{
            position: "fixed",
            right: 16,
            bottom: 36,
        ...
        >
          <a href={download.url} download={download.filename}>
            {download.revision !== editorRuntime.session?.inspect().revision
              ? "Bản cũ · "
              : ""}
            Tải kết quả bản {download.revision}
          </a>
        </aside>
      )}
    </>
