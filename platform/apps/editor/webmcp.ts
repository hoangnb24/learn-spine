...
const publish = (value: Download | null) => { download = value; for (const listener of listeners) listener(); };
export const getToolDownload = () => download;
export const subscribeToolDownload = (listener: () => void) => { listeners.add(listener); return () => { listeners.delete(listener); }; };
...
const unsubscribe = editorRuntime.subscribe(() => {
  ...
  if (next !== sessionId) { sessionId = next; publish(null); }
});
export const editorRegistration = registerWebMCP(editorBridge);
export async function disposeEditorTools() {
  unsubscribe(); publish(null);
  const result = await editorRegistration;
  if (result.ok) await result.value.dispose(); else editorBridge.dispose();
}
window.addEventListener('pagehide', event => { if (!event.persisted) void disposeEditorTools(); });
