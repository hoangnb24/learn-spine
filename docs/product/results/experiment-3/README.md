# Gate 3 — nine runs complete, final independent acceptance pending

Exactly nine fresh subjects were attempted. Robot1 failed on the browser/session environment before a native invocation and stays in the denominator. Robot2–3 and all three wave runs have independent acceptance. Three scarf runs have completed data, transaction and real ZIP reopen checks; their independent verdict and the aggregate gate verdict are pending. No replacement runs or solution repairs were made.
...
| [scarf-1](runs/scarf-1/README.md) | 57 + 1 | 193.3775 | Pending; partial public transcript |
| [scarf-2](runs/scarf-2/README.md) | 60 + 1 | 231.8685 | Pending; native viewport crop |
| [scarf-3](runs/scarf-3/README.md) | 56 + 1 | 246.0763 | Pending; manual count discrepancy audited |

Threshold remains at least2/3 per brief, every per-run criterion required, <=900s and <=100 calls. Robot is2/3 and wave3/3 by the recorded independent decisions. No scarf numerator or final Gate3 pass is assigned here. This is feasibility evidence, not statistical reliability, MVP readiness, native cancellation, or a performance claim.
...
For successful exports, timers run continuously from Start through Stop, including brief delivery and fixed interruption. Root's clock-after-send values are upper bounds, not invented exact delivery timestamps. Every run gets the exact checkpoint interruption from protocol; manual rescue is0. Raw call starts/completions count failed requests, retries, polling, artifact reads and saves. Discovery is reconciled with public transcript/root provenance. DOM observation and image presentation are not product calls. QA takes place after run Stop and cannot alter final ZIPs. Earlier QA overlap is recorded per run; no performance inference is made.
