...
if any((b/f'scarf-{n}'/'independent-review.json').exists() for n in (1,2,3)):
 raise SystemExit('Independent reviews now exist. This pre-review report generator is retained for provenance; do not overwrite reviewed audits.')
for n,first in [(1,16),(2,14),(3,15)]:
 ...
 limit='App public transcript stops after early inspect_project; no pagination cursor and final text missing. Root verified session and independently received subject final. Raw native starts/results/stop/download are complete; missing transcript is not backfilled.' if n==1 else ('The render_pose input viewport has35 outside geometry samples across the cycle. Preview fits its continuous envelope; this metric does not measure the preview viewport.' if n==2 else 'Subject manual count58 product vs56 raw; public code audit matches56. Image emission failed after successful measure_motion/render_pose; subsequent re-emission adds no native invocation. One discovery makes57 budget calls. No uncounted pre-dispatch call identified.')
 audit={'run':p.name,'outcome':'COMPLETED_PENDING_INDEPENDENT_REVIEW','publicInvocations':len(calls),'discoveryCalls':1,'budgetCalls':None if n==1 else len(calls)+1,'observedCalls':len(calls)+1,'elapsedMs':s['elapsedMs'],'withinBudget':None if n==1 else len(calls)+1<=100 and s['elapsedMs']<=900000,'unfinishedInvocations':s['unfinishedInvocations'],'transactionChecks':checks,'dataCriteria':score['dataCriteria'],'phase':score['phase'],'initialDiagnosticCount':measures[0]['result']['value']['total'],'finalDiagnosticCount':measures[-1]['result']['value']['total'],'finalZIP':score['finalFile'],'revision':3,'reopenChecks':{k:{'sameProjectAsStop':v['sameProjectAsStop'],'assetHashesMatch':v['assetHashesMatch'],'wraps':v['animations']['cycle']['wraps']} for k,v in reopen['results'].items()},'geometry':geo['animations']['cycle'],'limitation':limit,'subjectPreview':{'uniqueFrames':24,'fps':4,'loops':3},'visualAuthor':'Sampled Player frames03/09 show continuous intact scarf, anchored left seam, distinct tip motion; independent judgment pending. Two WebM recordings and24 UI images saved per run.','manualRescue':None if n==1 else 0,'scenarioFeedback':1,'tokens':None,'cost':None,'rawSHA256':hashlib.sha256((p/'events.jsonl').read_bytes()).hexdigest()}
...
{len(calls)+1} observed calls ({len(calls)} native + one discovery); complete budget for scarf1 unknown, {s['elapsedMs']/1000:.4f}s.
[Final revision3 ZIP]({score['finalFile']}) was opened through actual Editor and Player file inputs. Both full projects and asset hashes match the stop snapshot. Both playback recordings cross at least3 loops. Post-run QA does not change solutions; scarf1 complete rescue status is unknown.
